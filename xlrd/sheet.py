...
class Sheet(BaseObject):
    ...
    def row(self, rowx):
        ...
        """
        return [self.cell(rowx, colx) for colx in range(len(self._cell_values[rowx]))]
    ...
    def fake_XF_from_BIFF20_cell_attr(self, cell_attr, style=0):
        ...
        upkbits(
            xf.protection,
            prot_bits,
            ((6, 0x40, "cell_locked"), (7, 0x80, "formula_hidden"),),
        )
    ...
    def handle_hlink(self, data):
        ...
        if (options & 1) and not (
            ...
            if (
                ...
                offset += 4
                h.url_or_path = data[offset : offset + nbytes].decode("UTF-16le")
                if DEBUG:
                    fprintf(
                        self.logfile,
                        "initial url=%r len=%d\n",
                ...
                if sz:
                    ...
                    extended_path = data[offset : offset + xl].decode(
                        "UTF-16le"
                    )  # not zero-terminated
                    offset += xl
    ...
    def handle_txo(self, data):
        ...
        while totchars < cchText:
            ...
            nb = data2[0]  # 0 means latin1, 1 means utf_16_le
            nchars = data2_len - 1
            if nb:
                assert nchars % 2 == 0
                nchars //= 2
...
def unpack_RK(rk_str):
    ...
    if flags & 2:
        ...
        (d,) = unpack(
            "<d", b"\0\0\0\0" + chr(flags & 252).encode("latin1") + rk_str[1:4]
        )
        if flags & 1:
            return d / 100.0
        return d
