...
class X12Styles(X12General):
    ...
    def do_numfmt(self, elem):
        ...
        if self.verbosity >= 3:
            self.dumpout(
                "numFmtId=%d formatCode=%r is_date=%d", numFmtId, formatCode, is_date
            )
...
class X12Sheet(X12General):
    ...
    def do_dimension(self, elem):
        ...
        if ref:
            ...
            rowx, colx = cell_name_to_rowx_colx(last_cell_ref, allow_no_col=True)
        ...
        for cell_elem in row_elem:
            ...
            if cell_name is None:  # Yes, it's optional.
                ...
                if explicit_row_number and cell_name[charx:] != row_number:
                    raise Exception(
                        "cell name %r but row number is %r" % (cell_name, row_number)
                    )
            ...
            if cell_type == "n":
                ...
                for child in cell_elem:
                    ...
                    if child_tag == V_TAG:
                        ...
                        bad_child_tag(child_tag)
                self.sheet.put_cell(
                    rowx, colx, XL_CELL_BOOLEAN, cnv_xsd_boolean(tvalue), xf_index
                )
            elif cell_type == "e":
                # e = error
                ...
                raise Exception(
                    "Unknown cell type %r in rowx=%d colx=%d" % (cell_type, rowx, colx)
                )
